...
type lpResult struct {
	solution *models.ScheduleSolution
	err      error
}
...
func (s *LPScheduler) ScheduleIrrigation(ctx context.Context, req models.ScheduleRequest) (*models.ScheduleSolution, error) {
	...
	lpCtx, lpCancel := context.WithTimeout(ctx, time.Duration(lpTimeout)*time.Second)
	defer lpCancel()

	resultCh := make(chan lpResult, 1)
	go func() {
		sol := s.solveGreedyLP(candidates, field, req, elecCost)
		select {
		case resultCh <- lpResult{solution: sol}:
		case <-lpCtx.Done():
		}
	}()
	...
	select {
	case <-lpCtx.Done():
		return nil, fmt.Errorf("线性规划求解超时: %v", lpCtx.Err())
	case res := <-resultCh:
		if res.err != nil {
			return nil, res.err
		}
